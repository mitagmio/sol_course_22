...
contract OMD_LAUNCHPAD is Context, Ownable {
    ...
    function getInflTokenSize(bytes32 referalCode, bytes32 symbol) external view returns(uint256) {
    ...
    function stringSymbolToByte32(string memory symbol) public pure returns(bytes32) {
        return bytes32(bytes(symbol));
    }

    function byte32SymbolToString(bytes32 symbol) public pure returns(string memory) {
        uint8 i = 0;
    ...
    function getSwapTokenAddress(bytes32 symbol) external view returns(address) {
    ...
    function setWhitelistToken(bytes32 symbol, address tokenAddress, uint256 basePrice) external onlyOwner {
        whitelistedSymbols.push(symbol);
        whitelistedTokens[symbol] = tokenAddress;
        priceTokens[symbol][0] = basePrice;
    }

    function setSwaplistToken(bytes32 symbolFrom, bytes32 symbolTo, uint256 price) external onlyOwner {
        swapPairs[symbolFrom] = symbolTo;
        swapTokensPrice[symbolFrom][symbolTo] = price;
    }

    function setTierTokenPrice(bytes32 symbol, uint8 tier, uint256 price) external onlyOwner {
        priceTokens[symbol][tier] = price;
    }

    function setSwapTokenAddress(bytes32 symbol, address addrToken) external onlyOwner {
    ...
    function setReferalCode(bytes32 referalCode, uint16 percent, address addrInfl) external onlyOwner {
    ...
    function buyToken(bytes32 symbol, uint256 _amount, bytes32 referalCode) external{
        ...
        if (referalCode != bytes32("base")) {
            if (inflAddress[referalCode] != address(0)) {
                require(inflReflPercent[referalCode] > 0,"inflReflPercent[referalCode] must be greater than 0");
                uint256 refAmount = _amount * inflReflPercent[referalCode] / 1000;
                omd.transferFrom(_msgSender(), inflAddress[referalCode], refAmount);
        ...
        require(launchToken.balanceOf(address(this)) >= amount, "There are not so many tokens for sale.");
        ...
        emit BuyTokenRef(sender, _amount, strSymbol, price, strReferalCode, referalCode);
    }
    ...
    event BuyTokenRef(address indexed to, uint256 amount, string symbol, uint256 price, string referalCode, bytes32 indexed refCode);
}